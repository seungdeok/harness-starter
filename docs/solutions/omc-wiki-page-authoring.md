...
## 후속 (2026-08-02) — 이 wiki 에는 더 이상 쓰지 않는다

위 함정들을 다 고쳐 놓고 나서 더 근본적인 질문이 나왔다: **왜 `docs/` 를 두고 `.omc/wiki/` 에
썼는가.** 답은 "쓰면 안 됐다" 였다.

ADR-009 는 `.omc/wiki` 를 **축적지로 삼는 안을 이미 기각**했고("OMC 의 라이브 데이터스토어라
성립하지 않는다"), 커밋 대상으로 올린 건 순전히 자동 생성물을 **잃지 않기 위해서**였다.
그런데 issue #29 가 그 자동 생성물을 전부 무시 대상으로 뺐다 — 지키려던 대상이 사라진 것이다.
그 상태에서 손으로 페이지를 써 넣은 건 ADR 이 기각한 안을 되살린 셈이었다.

ADR-011 로 `.omc/` 를 전부 무시하고 지식 저장소를 `docs/` 로 단일화했다.
위 "부수 사항"의 결론(**자동 생성 파일은 생성된 그대로 커밋**)도 함께 무효다 — 커밋하지 않는다.

**이 노트의 두 교훈은 그대로 유효하다.** 파생 메타데이터가 본문과 갈라지는 것도,
슬러그를 예측해 참조를 미리 만드는 것도 wiki 밖에서 똑같이 일어난다.
