...
- [2026-08-02] squash 머지를 쓰는 레포에서 도달 가능성(`branch -d`·`git log A..B`·`git cherry`)으로 "머지됐나"를 판정하지 않는다 — 전부 "미머지"라고 답한다. 내용 비교로 가되 base 전진에 오염되지 않게 그 브랜치만 만드는 파일로 좁힌다. (근거: gate-ref-symmetry.md)
