...
- [ADR-014](#adr-014) — `CLAUDE.md` 템플릿을 가이드라인/문서참조 두 조각으로 쪼개고, 조각별 scope 를 허용하되 조합은 2개만 둔다 (issue #38)
...
**후속**: "`ARCHITECTURE.md` 가 빈 템플릿이라 구조·용어가 갈 곳이 없다"는 트레이드오프는 해소됐다 — `docs/solutions/pipeline.md`(해결 노트 형식을 안 따르는 레퍼런스 문서였다)를 `ARCHITECTURE.md` 로 옮겨 채웠다. 같은 정리에서 다른 프로젝트 노트(`click-cooldown.md`)를 지우고, ADR↔노트↔GUARDRAILS 상호참조를 실제 링크로 바꿨다(ADR 에 `<a id="adr-NNN">` 앵커 + 목차). 노트의 YAML frontmatter 는 **지우려다 되돌렸다** — 이 ADR 이 wiki 를 버렸으니 `tags` 의 소비자가 없다고 판단했으나, 소비자는 wiki 가 아니라 `/ce-compound` 자신이었다(다음 노트를 쓸 때 `tags:` 를 Grep 해 기존 노트 갱신 여부를 정한다). 지웠으면 같은 주제 노트가 중복으로 쌓였을 것이다. `solutions/README.md` 에 그 계약을 명시했다.
...
**후속 2**: "재실행마다 확인 프롬프트 1회"는 **2회가 됐다**(2026-08-05, ADR-014). `CLAUDE.md` 가 두 조각으로 나뉘면서 마커 행이 2개가 됐기 때문이다 — 조각이 서로 다른 파일에 갈 수 있으면 한 행으로는 상태를 표현할 수 없다. 여기서 "인정된 비용"으로 받아들인 그 비용이 두 배가 된 것이므로, 프롬프트 0회로 만드는 대안(마커 사이 내용을 템플릿과 문자열 비교)은 **여전히 미채택**이지만 재검토 압력은 그만큼 커졌다.
**관련 노트**: [procedure-level-changes.md](solutions/procedure-level-changes.md)

---

<a id="adr-014"></a>
### ADR-014: `CLAUDE.md` 템플릿을 두 조각으로 쪼개고, 조각별 scope 를 허용하되 조합은 2개만 둔다 (issue #38)
**결정**: `templates/CLAUDE-section.md` 를 `CLAUDE-guidelines.md`(§1~6 행동 가이드라인)와 `CLAUDE-docs.md`(`## Project Docs` @import 블록)로 **파일 단위 분리**하고, 마커도 `harness:guidelines`/`harness:docs` 두 쌍으로 나눈다. §1 의 CLAUDE.md 질문은 **개수를 늘리지 않고 선택지만** 바꿔 `둘 다 프로젝트` / `A 는 글로벌·B 는 프로젝트` / `건너뛰기` 세 개를 둔다 — **B 를 글로벌에 두는 선택지는 만들지 않는다.** §2 스캔의 마커 행은 2개가 되고, `harness:guidelines` 는 §1 답변과 **무관하게 프로젝트·글로벌 두 파일을 모두** 본다. 레거시 `<!-- harness:start -->` 판정이 두 신규 행보다 **먼저** 오며, 위치별로 담긴 내용이 다르다(프로젝트=A+B, 글로벌=A만). 함께 `TESTING.md` 템플릿(질문 형식)을 6번째 docs 템플릿으로 추가해 @import 에 넣고, §5 가 append 뒤 @import 6개 경로의 실재를 확인하며, A 를 글로벌에 넣은 경우 §6 이 제거 방법을 안내한다.
**이유**: `@` 는 **파일 위치 기준 상대경로**다(실측: `~/.claude/CLAUDE.md` 의 `@RTK.md` → `~/.claude/RTK.md`). 그래서 B 를 글로벌에 두려면 절대경로여야 하고, 그러면 **모든 프로젝트 세션에 특정 레포의 문서가 실린다** — 기술적으로 가능하지만 의미가 뒤집히므로 고를 수 없는 것을 선택지에 남기지 않는다(GUARDRAILS 2026-08-01). 파일 분리를 택한 것은, 한 파일을 `## Project Docs` 헤딩에서 잘라 쓰면 그 파싱 규칙이 산문에 생겨 템플릿을 편집할 때마다 조용히 깨지기 때문이다 — 이 레포의 산문은 **어떤 CI 도 검증하지 않는다**(`ci-check-coverage.md`). 파일 2개가 규칙 0개보다 싸다.
**트레이드오프**: **재실행 확인 프롬프트가 1회 → 2회**가 된다 — ADR-013 이 인정된 비용으로 명시한 것을 두 배로 만들며, 그쪽에 후속으로 적었다. 미러 규칙의 대상이 1개에서 2개로 늘어 `GUARDRAILS.md` 를 "해당하는 쪽"으로 고쳐야 했다. `@import` 가 4 → 6줄이 되어 매 세션 컨텍스트가 는다(`ADR.md`·`ARCHITECTURE.md` 가 각각 150줄대) → 비용이 체감되면 그때 다룰 축으로 남긴다. **조합을 늘리자 "이전 상태"의 가짓수도 늘어 파생 결함이 연달아 나왔다** — 마커 행 분리(F2)가 레거시 중복 append 를 만들었고(D10), 그 수정이 다시 "scope 를 바꿔 재실행하면 A 가 두 곳에 남는" 결함을 만들었다(D11). 셋 다 계획이 "이번 실행"만 보고 설계된 데서 나왔고, 리뷰 단계에서 하나씩 잡혔다. 산문이라 CI 가 못 덮으므로 임시 레포 probe 10케이스로 대신하며, "실제 에이전트가 이 산문을 읽고 같은 판정을 내리는가"는 ADR-013 과 마찬가지로 **미검증으로 남는다**. 검토했으나 버린 안: **한 파일 유지 + 헤딩에서 자르기**(검증 안 되는 파싱 규칙이 생긴다), **글로벌 B 를 절대경로로 허용**(위 이유), **§1 질문을 2개로 분리**(확인 지점이 늘어 ADR-013 의 "확인은 한 번만"과 부딪힌다), **레거시 자동 마이그레이션**("기존 파일을 덮어쓰지 않는다"의 예외를 늘린다 — 갱신을 명시적으로 고른 경우에만 교체), **setup 에 블록 제거 기능 추가**(설치 마법사가 관리 도구로 성격이 바뀐다 → 안내 1줄로 갈음), **마커 내용 비교 판정 부활**(ADR-013 이 요구한 "필요의 실증"이 아직 없다).
