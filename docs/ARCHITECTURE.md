...
이 레포는 **그 자체가 Claude Code plugin(`harness`)이자 marketplace** 예요 (ADR-002).
설치하면 어느 레포에서든 한 작업(phase)을 `discuss → … → make-pr` 순서로 도는 반자동
파이프라인을 쓸 수 있어요. 기본 단위는 **phase 하나 = 브랜치/worktree 하나 = 세션 하나** 이고,
여러 에이전트를 지휘하는 오케스트레이션 도구가 아니에요.

진행 상태는 `phases/<slug>/phase.json` 이 기억하고, `pipeline.py` 자체는 **의존성 0(stdlib)**
짜리 stage 체커예요. 승인 게이트(`discuss`·`approve`·커밋 범위 확인)는 생략할 수 없어요 — 그게
이 파이프라인의 목적이에요 (ADR-004).

## 디렉토리 구조

```
.claude-plugin/     plugin.json · marketplace.json (레포 = plugin = marketplace)
skills/
  pipeline/         파이프라인 스킬 + scripts/pipeline.py (단일 소스)
  setup/            설치 마법사 + templates/ (대상 레포로 뿌리는 scaffold 원본)
  make-pr/ make-issue/
.claude/skills/     gstack 벤더 스킬 (plan-ceo-review·plan-eng-review) — plugin 미포함
docs/               이 레포의 유일한 지식 저장소 (ADR-011)
  PRD.md ADR.md ARCHITECTURE.md
  solutions/        해결 노트 + GUARDRAILS.md
phases/<slug>/      phase 런타임 산출물 — 커밋하지 않아요 (ADR-005, CLAUDE.md §6)
```
...
> (출처 헤더 2줄이 붙어요 — ADR-008). 아래 예시의 `<pipeline.py>` 는 그 둘 중 하나의 경로예요.
> 스크립트는 **cwd 기준 git root** 에 phase 를 만들므로(스크립트 위치와 무관), worktree 안에서
> 실행하면 그 worktree 가 대상이에요.

## 데이터 흐름

### Stage 매핑

OMC/내부 스킬 우선. discuss/approve 는 스킬 없이 세션에서 사람이 직접 진행하는 대화형 stage.
compound 만 CLAUDE.md 5장이 `/ce-compound` 를 지정.

> **이 표를 고치면 `skills/pipeline/SKILL.md` 와 `pipeline.py` 의 `STAGES` 도 함께 갱신해요** —
> 런타임 판정은 그 둘이 하고, 이 표는 설명이에요.
...
| compound         | `/ce-compound`                           | 교훈을 `solutions/` 에 기록 (CLAUDE.md 5장)      |
...
> **compound** — 파이프라인은 `--no-compound` 를 늘 붙여 기본 제외해요 (ADR-001). 필요하면 종료 후
> `/ce-compound` 를 직접 실행하고, `done` 이 그 교훈이 `origin/<base>` 에 도착했는지 확인해요 (ADR-012).
...
빌드 단계가 없어요 — 스킬은 마크다운, `pipeline.py` 는 stdlib 파이썬이에요.

```bash
python3 <pipeline.py> selftest   # cursor/advance/slug 순수 로직 체크 (CI: validate.yml)
claude plugin validate .         # 매니페스트 검증 (CI: plugin-validate.yml, ADR-010)
claude --plugin-dir .            # 이 레포 안에서 스킬을 직접 써보기 (dogfooding)
```
